...
import csv # Імпортуємо модуль, який містить методи для формування списку з рядків файлу
...
class Netflix:
   ...
   def readFile(self): # Функція зчитування файлу
      ...
      except (BaseException): # Якщо помилка зчитування, то повідомаляємо про це і зупиняємо роботу скрипту
...
class Cast:
   ...
   def __next__(self): # Метод, який відповідає за покрокове повернення рядків
      if self.i == self.size: # Якщо кількість кроків дорівнює розміру, то зупиняємо 
         raise StopIteration
      while self.i < self.size: # Цей цикл потрібний щоб оминати ті рядки, які не відповідають нашим умовам
